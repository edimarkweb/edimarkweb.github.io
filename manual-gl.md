...
A primeira vez, a aplicación abre este manual nunha lapela. Cando o peches non se volverá abrir só: desde entón unha sesión baleira comeza cun documento en branco, e o manual segue á man en **Axuda** ou con `F1`. Nese mesmo menú está o [artigo do blog](https://educacion.bilateria.org/edimarkweb-escribir-en-markdown-y-entregar-en-cualquier-formato), que conta para que serve todo isto na clase, con exemplos e capturas; este manual explica como se fai cada cousa.
...
Con `Ctrl+V` ou o botón **Pegar**, EdiMarkdown coloca no panel axeitado o que haxa no portapapeis: o texto plano e o Markdown van ao editor Markdown, na posición do cursor; o contido con formato (Word, LibreOffice, unha páxina web, unha fórmula dun chatbot) e mesmo as imaxes recompóñense no editor visual e xeran o seu Markdown. Non fan falta pasos intermedios: copia de onde sexa e pega.
...
* **Configuración (Ctrl+,)**: **Idioma** da interface; **Tema** (Sistema, Claro ou Escuro, lémbrase); **Xanela independente**, que abre EdiMarkdown sen lapelas nin barra de enderezos (só na versión web); **Corrector ortográfico**, que subliña as faltas cos dicionarios do equipo e segue o idioma do documento; e **Opcións xerais…**.
* **Imprimir (Ctrl+P)**: unha vista lista para papel ou PDF.
* **Axuda**: o **Manual (F1)**, **Acerca de EdiMarkdown** —versión, autor e licenzas— e, no escritorio, **Buscar actualizacións…**.
...
En **Configuración → Opcións xerais… → Citas** podes cargar unha biblioteca **BibTeX** (`.bib`) ou **CSL JSON** (`.json`). **Cargar bibliografía de exemplo** prepara sete referencias completas; se xa tiñas bibliografía, súmanse á túa sen substituíla. Con **Engadir referencia…** podes ampliar a biblioteca cargada —incluída a de exemplo— ou crear unha nova. A clave de cita é opcional: se a deixas baleira, o programa compón unha co apelido, o ano e unha palabra do título, e asegúrase de que non coincida con outra. Admite artigos, libros, capítulos, informes, páxinas web, teses e comunicacións, cos campos específicos necesarios para compoñer correctamente as referencias finais. **APA 7** é o estilo inicial; tamén podes escoller Chicago autor-data, MLA 9, IEEE ou un ficheiro **CSL** propio. Tamén se poden cambiar o título da bibliografía e o nivel H1–H6. Os ficheiros non se envían a ningún servizo.

O botón do libro —ou `Ctrl+Alt+B`— abre un buscador por autor, título, ano ou clave, con **Engadir unha referencia manualmente** sempre á man para escribir unha nova sen saír do cadro. Ao pé do cadro lese con que estilo se van compoñer as citas —APA, Chicago, MLA, IEEE ou o CSL que cargases— e unha ligazón leva ás opcións da bibliografía para cambialo. A forma **parentética** produce `[@garcia2024]`; a **narrativa**, `@garcia2024`; e **só o ano**, `[-@garcia2024]`, cando o nome xa está na frase. Cunha soa referencia podes engadir páxinas ou outro localizador, como `[@garcia2024, p. 5]` ou `@garcia2024 [pp. 5–7]`. As citas múltiples usan a forma parentética. Se o cursor está dentro dunha cita, o mesmo botón permite editar todos estes datos. A bibliografía final aparece na vista previa e en todas as exportacións.

Ao gardar `o-meu-ficheiro.md`, EdiMarkdown copia a biblioteca en `o-meu-ficheiro/references.bib` —ou `references.json`— e declara esa ruta nos metadatos YAML. As imaxes propias agrúpanse en `o-meu-ficheiro/images/`. Para trasladar o traballo abonda con conservar xuntos o Markdown e o seu cartafol `o-meu-ficheiro`. Se usas un ficheiro CSL propio, cópiase ao lado, en `o-meu-ficheiro/style.csl`, e tamén queda declarado: así as citas vense igual en calquera equipo. A aplicación de escritorio recupera a biblioteca e mais o estilo automaticamente; na versión web, por seguridade, hai que vincular o cartafol do documento —o aviso **Buscar o seu cartafol…** que aparece sobre a vista previa— ao abrilo noutro navegador ou ordenador.
...
As ligazóns seguen o tipo de letra do documento. Ao elixir **Sen remates**, as letras latinas e as cifras das fórmulas tamén se amosan sen serifa na vista previa, no PDF e nas exportacións. Os símbolos especiais e os alfabetos indicados expresamente na fórmula conservan a súa tipografía matemática. O código Markdown orixinal non cambia.
...
**Perfís de formato.** Se repites os mesmos axustes en varios traballos, gárdaos unha vez: na parte de arriba do cadro, *Gardar como perfil…* pide un nome —«TFG», «apuntamentos»— e lembra o texto, a páxina e o índice tal como estean nese momento. Noutro documento abonda con escollelo e premer *Aplicar o perfil*: enche os campos que o perfil fixa e deixa os demais como están, así que aínda podes retocar antes de confirmar. Nada chega ao documento ata que premas *Aplicar*, e *Cancelar* descártao como sempre. Gardar co nome dun perfil que xa existe actualízao. Os perfís quedan neste equipo, non viaxan dentro do `.md`.
...
En **Configuración → Almacenamento…** vese o que EdiMarkdown garda neste navegador: cantos documentos tes abertos, o que ocupa o seu texto gardado automaticamente e cantas imaxes hai, co seu tamaño. Nada disto sae do teu equipo.

Á parte cóntanse as **imaxes sen documento**, que son as de lapelas pechadas hai tempo ou de sesións anteriores. Bórranse soas ao abrir a aplicación, e o botón **Borrar as imaxes sen documento** faino no momento; é útil despois de importar varios PDF, que son os que máis ocupan. **Esquecer a sesión gardada…** pecha as lapelas e descarta o texto gardado coas súas imaxes, tras preguntar. Ningunha das dúas cousas borra ficheiros do teu disco.
...
**A primeira vez en macOS.** O `.dmg` non está asinado cunha conta de desenvolvedor de Apple, así que ao abrilo macOS avisa de que a aplicación «está danada e debería moverse ao lixo». Non o está: é o que di o sistema ante calquera aplicación sen asinar descargada de internet. Arrastra EdiMarkdown a **Aplicacións**, abre o **Terminal** e executa `xattr -dr com.apple.quarantine /Applications/EdiMarkdown.app`. Con iso ábrese con normalidade e non hai que repetilo: esa orde só quita a marca de «descargado de internet» a esa aplicación, sen cambiar ningún axuste de seguranza do sistema.

Fronte ao navegador engade:

* **Dobre clic para abrir**: os ficheiros `.md` e `.markdown` quedan asociados, amosan a icona de EdiMarkdown no xestor de ficheiros e ábrense na aplicación; se xa está aberta, o documento chega a esa mesma xanela, que se pon diante. E se ese ficheiro xa estaba aberto, volve á súa lapela en vez de duplicarse. (A icona instálana o paquete `.deb` e os instaladores de Windows; a AppImage non toca o sistema.)
...
Escolle un PDF en **Importar**, ou arrástrao ao editor. A conversión faise no teu equipo, sen enviar o documento a un servidor. Ao abrir o cadro, o conversor le o PDF e mostra cantas páxinas contén antes de permitir xerar a vista previa.

Podes eliminar cabeceiras e pés repetidos, conservar imaxes e fórmulas detectadas como imaxes, aplicar OCR ás páxinas dixitalizadas e escoller o idioma, e indicar páxinas como `1-3, 5` (baleiro significa todas). A opción do OCR só aparece se o documento ten páxinas dixitalizadas, e o aviso di cantas son e en que páxinas están, agrupadas en intervalos; o seu idioma, só se a marcas. As dúas primeiras están activadas inicialmente; o OCR está desactivado, porque descarga un motor de recoñecemento e retarda a conversión. O idioma inicial do OCR coincide co da interface. Preme **Converter a Markdown**, revisa o resultado e despois **Importar nunha pestana nova**. A vista previa amosa o principio do documento e as súas doce primeiras imaxes, e só nomea as restantes; o documento impórtase enteiro. Cambiar as opcións require converter de novo. Cancelar non altera os documentos abertos. Unha barra mostra o progreso ao abrir o conversor, páxina a páxina, durante o recoñecemento, ao preparar a vista previa e ao gardar as imaxes do documento e ao montalo na súa lapela —primeiro o texto e despois a folla, varios segundos cada un nun informe longo—.

A detección de táboas, columnas e fórmulas pode dividir celas ou perder estrutura. O OCR produce texto editable, pero as fotografías, liñas, baixa resolución e marcas de auga poden causar erros; só se aplica ás páxinas dixitalizadas: as que non teñen texto extraíble e as que apenas conservan unha referencia ou un número sobre unha imaxe que ocupa case toda a folla. Cando o recoñecemento non achega máis do que a páxina xa tiña, como nunha lámina ou nunha portada cunha imaxe grande, consérvase a conversión normal coa súa imaxe e o seu texto en lugar dunha páxina baleira. Cando si achega máis, a páxina pasa a ser ese texto e a súa imaxe non se importa. As fórmulas detectadas son imaxes, non ecuacións editables. Desactiva a eliminación de cabeceiras e pés se desaparece texto útil. A versión web descarga e garda na caché o motor e o idioma no primeiro uso; a aplicación de escritorio inclúeos para traballar sen conexión. Límite: 50 MB por ficheiro, sen límite de páxinas; a partir dun centenar o diálogo avisa de que pode tardar minutos, e a mensaxe de avance di cantos quedan segundo o ritmo real. Desbloquea antes os PDF con contrasinal. A conversión só se interrompe se pasan tres minutos sen ningún avance.
