...
En **Configuración → Opcións xerais… → Citas** podes cargar unha biblioteca **BibTeX** (`.bib`) ou **CSL JSON** (`.json`). Se só queres probar a función, **Cargar bibliografía de exemplo** prepara sete referencias completas sobre educación, competencia dixital e aprendizaxe. Con **Engadir artigo…** podes ampliar a biblioteca cargada —incluída a de exemplo— ou crear unha nova indicando clave, autoría, título, revista, ano e, opcionalmente, DOI ou URL. **APA 7** é o estilo inicial; tamén podes escoller Chicago autor-data, MLA 9, IEEE ou cargar un ficheiro **CSL** (`.csl`) propio. Nestas opcións tamén se poden cambiar o título da bibliografía —«Referencias» por defecto— e o nivel H1–H6. Os ficheiros non se envían a ningún servizo.
