...
- PDF import: `pdf-import.js` owns the options/preview dialog; `pdf-worker.js` runs the vendored Pyodide/PyMuPDF runtime in `vendor/pdf-runtime/`; `pdf-import.py` handles page ranges, repeated margin text and detected math images. No OCR or native Layout model. Runtime URLs and SHA-256 pins are in its manifest; the Node test checks asset integrity. The browser regression uses a synthetic PDF in `test/fixtures/` (regenerator in `test/helpers/make-pdf-fixture.py`), never personal documents.
