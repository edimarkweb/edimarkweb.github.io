...
* **Menú de fórmulas** (en el editor Markdown): `Ctrl+M` abre la espera —la barra de estado recuerda las teclas— y después `1`, `2`, `3` o `4` elige el delimitador (`\(...\)`, `\[...\]`, `$...$` o `$$...$$`); `Intro` inserta `\(...\)`, que es el recomendado, y `Esc` cancela.
...
Los enlaces siguen el tipo de letra del documento. Al elegir **Sin remates**, las letras latinas y las cifras de las fórmulas también se muestran sin serifa en la vista previa, el PDF y las exportaciones. Los símbolos especiales y los alfabetos indicados expresamente en la fórmula conservan su tipografía matemática. El código Markdown original no cambia.
...
En **Importar**, elige un PDF (también puedes arrastrarlo al editor). La conversión se realiza en tu equipo, sin enviar el documento a un servidor. Al abrir el cuadro, el conversor lee el PDF y muestra cuántas páginas contiene antes de permitir generar la vista previa.

Puedes eliminar encabezados y pies repetidos, conservar imágenes y fórmulas detectadas como imágenes, aplicar OCR a las páginas escaneadas y elegir su idioma, e indicar páginas como `1-3, 5` (vacío significa todas). La opción del OCR solo aparece si el documento tiene páginas escaneadas, y el aviso dice cuántas son y en qué páginas están, agrupadas en intervalos; su idioma, solo si la marcas. Las dos primeras vienen activadas; el OCR viene desactivado, porque descarga un motor de reconocimiento y ralentiza mucho la conversión. El idioma inicial del OCR coincide con el de la interfaz. Pulsa **Convertir a Markdown**, revisa el resultado y después **Importar en una pestaña nueva**. La vista previa enseña el principio del documento y sus doce primeras imágenes, y solo nombra las demás; el documento se importa entero. Cambiar las opciones exige volver a convertir. Cancelar no altera los documentos abiertos. Una barra bajo el mensaje va diciendo por dónde va: primero al abrir el conversor y luego página a página, también durante el reconocimiento, al preparar la vista previa y al guardar las imágenes del documento y al montarlo en su pestaña —el texto primero y la hoja después, que en un informe largo son varios segundos cada uno—, así que en un documento largo se ve que sigue trabajando y nunca parece detenido.

La detección de tablas, columnas y fórmulas no es infalible: puede dividir celdas o perder estructura. El OCR produce texto editable, pero fotografías, líneas, baja resolución y marcas de agua pueden causar errores; se aplica solo a las páginas escaneadas, es decir, las que no tienen texto extraíble y las que apenas conservan una referencia o un número sobre una imagen que ocupa casi toda la hoja. Cuando el reconocimiento no aporta más que lo que la página ya contenía, como en una lámina o en una portada con una imagen grande, se conserva la conversión normal con su imagen y su texto en lugar de una página vacía. Cuando sí aporta más, la página pasa a ser ese texto y su imagen no se importa. Las fórmulas detectadas se guardan como imágenes, no como ecuaciones editables. Si desaparece texto útil, desactiva la eliminación de encabezados y pies. La primera página reconocida descarga y guarda en caché el motor y el idioma en la versión web; el escritorio los incluye para trabajar sin conexión. Se admiten archivos de hasta 50 MB, sin límite de páginas; a partir de un centenar el diálogo avisa de que puede tardar minutos, y el mensaje de avance dice cuántos quedan según el ritmo real. Los PDF con contraseña deben desbloquearse antes. La conversión solo se interrumpe si transcurren tres minutos sin ningún avance, no por su duración total.

Las imágenes del PDF se guardan automáticamente como recursos separados, para no llenar el autoguardado del navegador con datos Base64. Se recuperan al volver a abrir la aplicación y se incluyen al guardar el Markdown con sus recursos o al exportar.
