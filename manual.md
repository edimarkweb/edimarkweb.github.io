...
**La lupa** de la barra de estado (`−`, el porcentaje y `+`, o `Ctrl` + `+` / `Ctrl` + `-`) agranda o reduce lo que ves en el panel activo; el porcentaje vuelve al 100 %. Agranda la hoja entera, con sus páginas y sus márgenes, así que la página no se reordena: si deja de caber en el panel, aparece una barra para recorrerla. No cambia el documento ni lo que se exporta o imprime —el papel sale siempre al 100 %—: el tamaño de letra está en *Formato del texto*.

**Las páginas**: la hoja mide lo que mide el papel —A4 o Carta, lo que diga el documento— y el editor visual reparte el texto en páginas, con su hueco entre una y otra. El corte cae siempre entre dos bloques, nunca a media línea: lo que no cabe al final de una página pasa entera a la siguiente, como en un procesador de textos. Es fiel al PDF y a la impresión, que salen de esta misma hoja y cortan por donde corta ella; para Word o LaTeX es orientativo, porque cada uno reparte las líneas a su manera. Si la hoja no cabe entera —una ventana estrecha, o los dos paneles a la vez—, el reparto se retira hasta que vuelva a caber: basta con bajar la lupa.
...
La barra de herramientas, bajo la anterior, reúne deshacer y rehacer, negrita, cursiva, encabezados (H1…H6), listas, citas, código, enlaces, imágenes, tablas, **Pegar** y las fórmulas. Cada botón dice al pasar el ratón qué hace y con qué atajo. En pantallas pequeñas se pliega en dos botones, **Acciones** y **Formato**.
...
**Imágenes incrustadas.** Cuando llegan dentro del texto —al importar un DOCX, al pegar desde otra aplicación—, su código ocupa miles de caracteres. EdiMarkWeb las pliega: en el editor queda una marca corta como `__EDIMARK_B64_1__` y, bajo el panel, una lista con la miniatura, el formato y el tamaño de cada una, con botones para **verla**, **ver su código** o **eliminarla**. El contenido real se conserva intacto al guardar, copiar y exportar.

El botón **Pasar las imágenes a la carpeta**, en esa misma lista, hace el camino de vuelta: cada imagen se convierte en un archivo dentro de una carpeta con el nombre del documento (`mi-archivo.md` → `mi-archivo/`) y en el Markdown queda su ruta. Los archivos se escriben al guardar, y `Ctrl+Z` deshace el cambio.
