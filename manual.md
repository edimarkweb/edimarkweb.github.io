...
**La lupa** de la barra de estado (`−`, el porcentaje y `+`, o `Ctrl` + `+` / `Ctrl` + `-`) agranda o reduce lo que ves en el panel activo; el porcentaje vuelve al 100 %. No cambia el documento ni lo que se exporta o imprime —el papel sale siempre al 100 %—: el tamaño de letra está en *Formato del texto*.
...
* **Formato**: los mismos ajustes de texto de la sección anterior, como valores de partida. Tres vienen puestos —**12 pt**, **con remates** e interlineado **1,5**—, porque son los que la vista previa necesita para enseñar la verdad: declarados, lo que se ve en la hoja es lo que sale en los cinco formatos. Los demás salen sin fijar.
