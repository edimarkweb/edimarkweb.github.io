...
**La lupa** de la barra d'estat (`−`, el percentatge i `+`, o `Ctrl` + `+` / `Ctrl` + `-`) engrandeix o redueix el que veus al plafó actiu; el percentatge torna al 100 %. Engrandeix el full sencer, amb les seves pàgines i els seus marges, així que la pàgina no es reordena: si deixa de cabre al plafó, apareix una barra per recórrer-la. No canvia el document ni el que s'exporta o s'imprimeix —el paper surt sempre al 100 %—: la mida de la lletra és a *Format del text*.

**Les pàgines**: el full amida el que amida el paper —A4 o Carta, el que digui el document— i l'editor visual reparteix el text en pàgines, amb el seu buit entre una i l'altra. El tall cau sempre entre dos blocs, mai a mitja línia: el que no cap al final d'una pàgina passa sencer a la següent, com en un processador de textos. És fidel al PDF i a la impressió, que surten d'aquest mateix full i tallen per on talla ell; per al Word o el LaTeX és orientatiu, perquè cadascun reparteix les línies a la seva manera. Si el full no hi cap sencer —una finestra estreta, o els dos plafons alhora—, el repartiment es retira fins que hi torni a cabre: n'hi ha prou de baixar la lupa.
