...
Si prefereixes que les imatges no vagin dins del text, el botó **Passar les imatges a la carpeta**, en aquest mateix plafó, fa el camí de tornada: cada imatge es converteix en un fitxer d'`imagenes/` i al Markdown queda la seva ruta, que és com desa les imatges qualsevol `.md`. Els fitxers s'escriuen en desar el document, pel mateix camí que la resta d'imatges amb ruta: al costat del `.md` a l'aplicació d'escriptori, a la carpeta que triïs al navegador i, on el navegador no ofereix triar carpeta, dins del ZIP. Mentrestant se segueixen veient a la vista prèvia, i `Ctrl`+`Z` desfà el canvi si no era el que volies.
