...
With `Ctrl+V` or the **Paste** button, EdiMarkdown puts whatever is on the clipboard into the right pane: plain text and Markdown go to the Markdown editor, at the cursor position; formatted content (Word, LibreOffice, a web page, a formula from a chatbot) and even images are rendered in the visual editor and generate their Markdown. No intermediate steps: copy from wherever and paste.
...
Under **Settings → General document options… → Citations**, load a **BibTeX** (`.bib`) or **CSL JSON** (`.json`) library. **Load example bibliography** prepares seven complete references; if you already had a bibliography, they are added to it instead of replacing it. Use **Add reference…** to expand the loaded library—including the example—or create a new one. The citation key is optional: leave it empty and the program builds one from the surname, the year and a word of the title, making sure it does not clash with another. Journal articles, books, chapters, reports, web pages, theses and conference papers are supported, with the specific fields needed to compose the final references correctly. **APA 7** is the initial style; you can also choose Chicago author-date, MLA 9, IEEE, or load your own **CSL** (`.csl`) file. You can also change the bibliography title and its H1–H6 level. Files are not sent to any service.

The book button —or `Ctrl+Alt+B`— opens a search by author, title, year or key, with **Add a reference manually** always at hand to write a new one without leaving the dialog. At the foot of the dialog you can read which style the citations will use—APA, Chicago, MLA, IEEE or the CSL file you loaded—and a link opens the bibliography options to change it. The **parenthetical** form produces `[@garcia2024]`; the **narrative** form, `@garcia2024`; and **year only**, `[-@garcia2024]`, for a name already written in the sentence. With one reference you can add pages or another locator, such as `[@garcia2024, p. 5]` or `@garcia2024 [pp. 5–7]`. Multiple citations use the parenthetical form. If the cursor is inside a citation, the same button edits all these details. The final bibliography appears in the preview and all exports.

When `my-file.md` is saved, EdiMarkdown copies the library to `my-file/references.bib`—or `references.json`—and declares that path in the YAML metadata. Its own images are grouped under `my-file/images/`. To move the work, keep the Markdown and its `my-file` folder together. If you use your own CSL file, it is copied beside it, as `my-file/style.csl`, and declared too, so the citations look the same on any computer. The desktop app retrieves the library and the style automatically; in the web version, for security, link the document's folder—the **Find its folder…** notice above the preview—after opening it in another browser or computer.
...
Links follow the document font. Choosing **Sans serif** also displays Latin letters and digits in formulas without serifs in the preview, PDF and exports. Special symbols and alphabets explicitly selected in a formula retain their mathematical typeface. The original Markdown code does not change.
...
**Settings → Storage…** shows what EdiMarkdown keeps in this browser: how many documents you have open, how much their autosaved text takes up, and how many images there are, with their size. None of it leaves your computer.
...
**Updates**: on startup it checks once a day for a newer version and, when there is one, a notice appears with **Download and install**, which fetches the installer and launches it. Since no installer can replace the files of a running application, the same notice offers **Close EdiMarkdown**, which saves and quits. With an AppImage, the application downloads the new one and opens its folder so you can replace the old file. You can ask for the check whenever you like from **Help → Check for updates…**, or turn it off with the **Check on startup** box.
...
Choose a PDF in **Import**, or drag it into the editor. Conversion runs on your device without uploading the document to a server. When the dialog opens, the converter reads the PDF and displays its page count before allowing a preview to be generated.

You can remove repeated headers and footers, keep images and detected formulas as images, apply OCR to scanned pages and choose its language, and select pages such as `1-3, 5` (leave empty for all pages). The OCR option only appears when the document has scanned pages, and the notice says how many and which pages they are, grouped into ranges; its language, only when you tick it. The first two are enabled initially; OCR is off, because it downloads a recognition engine and slows the conversion down. The initial OCR language follows the interface language. Click **Convert to Markdown**, review the result, then **Import into a new tab**. The preview shows the beginning of the document and its first twelve pictures, and only names the rest; the document is imported whole. Changing options requires converting again. Cancelling leaves open documents unchanged. A bar tracks progress while the converter opens, page by page, during recognition, while the preview is prepared and while the document's images are saved and while it is mounted in its tab — the text first, the sheet after, each of them several seconds on a long report.

Table, column and formula detection can split cells or lose structure. OCR produces editable text, but photographs, rules, low resolution, and watermarks can cause errors; it is applied only to scanned pages: those without extractable text, and those that keep just a reference or a number over an image covering almost the whole sheet. When recognition adds nothing to what the page already held, as in a full-page illustration or a cover with a large image, the ordinary conversion is kept, with its image and its text, instead of an empty page. When it does add more, the page becomes that text and its image is not imported. Detected formulas are images, not editable equations. Disable header/footer removal if useful text disappears. The web version downloads and caches the engine and language on first use; the desktop app includes them for offline use. Limit: 50 MB per file, with no page limit; past a hundred pages the dialog warns that it may take minutes, and the progress message says how many are left from the pace it is actually keeping. Unlock password-protected PDFs first. Conversion is interrupted only after three minutes without progress, not because of its total duration.
