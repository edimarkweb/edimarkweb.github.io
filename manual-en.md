...
If you would rather the images did not travel inside the text, the **Move the images to the folder** button in that same panel does the return trip: each image becomes a file in a folder named after the document —`my-file.md` moves them to `my-file/`, with spaces turned into hyphens— and the Markdown keeps its path, which is how any `.md` stores images. The folder carries the document's name so that two `.md` files saved side by side do not overwrite each other's images, numbered from `01` in each one. The files are written when you save the document, along the same road as the rest of the images with a path: next to the `.md` in the desktop application, in the folder you choose in the browser and, where the browser offers no folder picker, inside the ZIP. Meanwhile they are still shown in the preview, and `Ctrl`+`Z` undoes the change if it was not what you wanted.
