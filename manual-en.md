...
Under **Settings → General document options… → Citations**, load a **BibTeX** (`.bib`) or **CSL JSON** (`.json`) library. If you only want to try the feature, **Load example bibliography** prepares seven complete references on education, digital competence and learning. Use **Add article…** to expand the loaded library—including the example—or create a new one by entering its key, authors, title, journal, year and, optionally, DOI or URL. **APA 7** is the initial style; you can also choose Chicago author-date, MLA 9, IEEE, or load your own **CSL** (`.csl`) file. These options also let you change the bibliography title—“References” by default—and its H1–H6 level. Files are not sent to any service.
