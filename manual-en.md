...
You can remove repeated headers and footers, keep images and detected formulas as images, apply OCR to scanned pages and choose its language, and select pages such as `1-3, 5` (leave empty for all pages). The OCR option only appears when the document has scanned pages, and the notice says how many; its language, only when you tick it. The first two are enabled initially; OCR is off, because it downloads a recognition engine and slows the conversion down. The initial OCR language follows the interface language. Click **Generate preview**, review the result, then **Import into a new tab**. The preview shows the beginning of the document and its first twelve pictures, and only names the rest; the document is imported whole. Changing options requires a new preview. Cancelling leaves open documents unchanged. A bar tracks progress while the converter opens, page by page, during recognition, while the preview is prepared and while the document is created.
