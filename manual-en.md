...
You can remove repeated headers and footers, keep images and detected formulas as images, apply OCR to scanned pages and choose its language, and select pages such as `1-3, 5` (leave empty for all pages). All three options are enabled initially. The initial OCR language follows the interface language. Click **Generate preview**, review the result, then **Import into a new tab**. Changing options requires a new preview. Cancelling leaves open documents unchanged. A bar tracks progress while the converter opens, page by page, and during recognition.

Table, column and formula detection can split cells or lose structure. OCR produces editable text, but photographs, rules, low resolution, and watermarks can cause errors; it is applied only to pages without extractable text. Detected formulas are images, not editable equations. Disable header/footer removal if useful text disappears. The web version downloads and caches the engine and language on first use; the desktop app includes them for offline use. Limits: 50 MB per file and 200 pages per conversion. Unlock password-protected PDFs first. Conversion is interrupted only after three minutes without progress, not because of its total duration.
